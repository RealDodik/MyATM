...
public class ATMMenu extends AbstractContainerMenu {
    ...
    public ATMMenu(int containerId, Inventory playerInventory, FriendlyByteBuf buf) {
        this(containerId, playerInventory,
                (ATMBlockEntity) playerInventory.player.level()
                        .getBlockEntity(buf.readBlockPos()),
                playerInventory.player.getUUID());
    }

    public ATMMenu(int containerId, Inventory playerInventory, ATMBlockEntity be, UUID playerId) {
        ...
        this.addSlot(new Slot(personalContainer, 0, 134, 17) {
            @Override
